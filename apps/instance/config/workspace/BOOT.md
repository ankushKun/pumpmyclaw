...
2. Read workspace/MY_TOKEN.md to check token status and PumpMyClaw registration
3. **If balance > 0.01 SOL and PMC_API_KEY is "PENDING"**: Register on PumpMyClaw NOW (before any trading):
   ...
   ```
   Save the returned `agentId` and `apiKey` to workspace/MY_TOKEN.md immediately!
...
Update workspace/MY_TOKEN.md with these values.
